...
*/
	function	createParser( $fname )
	{
		...
		{
			//	Read one line
			if (get_magic_quotes_runtime()) $line = stripslashes( fgets( $fp, 4096 ) );
			else $line = fgets( $fp, 4096 );

			//	check, wether leading and trailing whitepaces should be stripped
			if( isset( $this->whitespace[( count( $this->whitespace )-1 )] ) ) ## added by rk
			...
			if	( eregi( "<patTemplate:([[:alnum:]]+)[[:space:]]*(.*)>", $line, $regs ) )			
			{
				...
				if( isset( $attributes['keep'] ) && $attributes['keep'] > 0 ) ## mod. by rk
				{
	...
	function	startElementHandler( $fname, $tagname, $attributes, $line, $lineno )
	{
		...
		switch( $tagname )
		{
			...
			case "tmpl":
				...
				if( isset( $attributes['type'] ) && $tmpl_type	=	strtoupper( $attributes['type'] ) )
				{
				...
				if( isset( $attributes['src'] ) )
				{
					...
					if( isset($attributes['parse'] ) && $attributes['parse'] == "on" )
				...
				if	( $this->depth > 0 )
				{
					...
					if( isset( $attributes['placeholder'] ) && $placeholder = strtoupper( $attributes['placeholder'] ) )
					{
...
*/
	...
	{
		$this->globals[strtoupper($name)]	=	stripslashes((string)$value);
	}
...
*/
	function	parseStandardTemplate( $name, $mode="w" )
	{
		...
		if( isset( $this->parsed_templates[$name] ) )
		{
			if( $mode=="a" )
...
*/
	function	getParsedTemplate( $name="" )
	{
		...
		if	( $name!="" )
		{
			...
			// Next line edited by Alex Koshel
			if (isset($this->variables[$name]) && (count($this->variables[$name])>0 || isset($this->dependencies[$name])))
			     $this->parseTemplate( $name );
			
			//	And return the parsed template
			if (isset($this->parsed_templates[$name])) return $this->parsed_templates[$name];
			else return "";
		}
	...
	function	getTemplateContent( $name )
	{
		...
		switch	( $this->getAttribute( $name, "type" ) )
		{
			...
			case patTEMPLATE_TYPE_CONDITION:
				...
				if( isset( $this->plain_templates[$name][$conditionval] ) )
...
*/
	function	getVar( $template, $var )
	{
		...
		elseif( isset( $this->variables[$template][$var] ) )
...
*/
		...
		for	( $i = 0; $i < $this->cnt_templates; $i++ )
		{
			...
			if	( $this->cnt_subtemplates[$name] > 0 )
			{
				...
				for	( $j = 0; $j < $this->cnt_subtemplates[$name]; $j++ )
				{
					...
					echo	"					<td class=\"text\" valign=\"top\"><pre>".htmlspecialchars( $this->getPlainSubTemplate( $name, $condition ), ENT_QUOTES, 'cp1251' )."</pre></td>\n";
					echo	"				</tr>\n";
			...
			else
			{
				...
				echo	"		<td class=\"mid\" valign=\"top\"><b>Template Data:</b></td>\n";
				echo	"		<td class=\"text\" valign=\"top\">\n";
				echo	"			<pre>".htmlspecialchars( $this->getPlainTemplate( $name ), ENT_QUOTES, 'cp1251' )."</pre>\n";
				echo	"		</td>\n";
				echo	"	</tr>\n";
				...
				if( isset( $unused  ) && is_array( $unused ) && count( $unused ) > 0 )
				{
					echo	"	<tr>\n";
					echo	"		<td class=\"mid\" valign=\"top\"><b>Unused variables</b></td>\n";
					echo	"		<td class=\"text\" valign=\"top\">\n";
	...
	function	getAnyParsedTemplate( $name="" )
	{
		$name	=	strtoupper( $name );
		...
		else
		{
			//	The template uses dependencies, then start with the root template
			if	( $this->uses_dependencies )
				return	$this->getAnyParsedTemplate( $this->templates[0] );
			//	Only one template => parse and return it
			elseif( $this->cnt_templates==1 )
				return	$this->getAnyParsedTemplate( $this->templates[0] );
			//	No dependencies, but more than one => return all parsed templates in an array
			else
			{
				for	( $i = 0; $i < $this->cnt_templates; $i++ )
					{
					$arr[$this->templates[$i]]	=	$this->getAnyParsedTemplate( $this->templates[$i] );
					}
				return	$arr;
			}
		}
...
/********************************************************************
 *                   METHODS USED FOR SMARTMVC                      *
 ********************************************************************/
    /**
     * parser for simple tables
     *
     * Applies the content of the array $values to the template $template.
     * It is necessary that the names of the template vars matches the keys
     * of the $values array e.g.:
     * $values[$i]['col_name'] fills the template var {COL_NAME}
     * The methods checks whether there are records in $values and if so
     * it thats the visibility of the template to visisible. Otherwise
     * it set's the visibility of the message_template to visible. So you
     * dont have check whether $values contains any records but you have to 
     * define a message template.
     *
     * @author  Ralf Kramer
     * @param   string  template (name of the template to parse)
     * @param   array   values ( an 2dim array like: $val[$i]['col_name'] )
     * @return  void
     */    
    function parseRepeatedTemplate( $template, $values )
    { 
        $this->doEmptyRecordDetection( $template, $values );
        $i = 0;
        foreach ($values as $k => $v) {
            if( $i % 2 )
                $this->addVar( $template, "CLASS",  ALT_ROW1 );
            else
                $this->addVar( $template, "CLASS",  ALT_ROW2 ); 

            foreach( $v as $key => $value )
                $this->addVar( $template, strtoupper( $key ), $value );   
                    
            $this->parseTemplate( $template, "a" );
            $i++;
        }
    } 
    
    function simpleTemplateParser( $template, $values, $parse = true)
    { 
    ...
    function addHiddenField( $name, $value, $template )
    {
        $field = "<input type=\"hidden\" name=\"$name\" value=\"$value\">"; 
        $this->addVar( $template, strtoupper($name), $field );        
    }
     
    function parseDropdown( $template, $data, $selected )
    { 
        for( $i = 0; $i < count( $data ); $i++ )
        {
            ...
            if( $data[$i]['value'] == $selected )
                ...
                $this->addVar( $template, "SELECTED", "" );
                
            $this->parseTemplate( $template, "a" );
        }
    }
    
    function parse3ColTable( $template, $values )
    { 
        for( $i = 0; $i < count( $values ); $i++ )
        {
            $this->parseMultipleColRecord( $template, $values[$i],   1 );
            if( isset( $values[$i+1] ) )
                $this->parseMultipleColRecord( $template, $values[$i+1], 2 );
            if( isset( $values[$i+2] ) )
                $this->parseMultipleColRecord( $template, $values[$i+2], 3 );
    ...
    function parseMultipleColRecord( $template, $record, $index )
    {
        foreach( $record as $key => $value )
            $this->addVar( $template, strtoupper( $key . "_" . $index ), $value );
    }

    /**
     * parses a table with included dropdown menue
     *
     * Works in the manner of smartTemplateParser() 
     * But if one the values is an array the method assumes an dropdown menue
     * and redirects the array to the parseDropdown() method
     *
     * @author  Ralf Kramer
     * @param   string  $template (name of the template to parse)
     * @param   array   $values ( an 2dim array like: $val[$i]['col_name'] )
     * @param   string  $dropdown_template (name of the dropdown template to parse)
     * @param   string  $selected which record in the dropdown is selected
     * @return  void
     */    
    function parseDropdownTable( $template, $values, $dropdown_template, $selected )
    { 
        for( $i = 0; $i < count( $values ); $i++ )
        {
            if( $i % 2 )
            ...
            foreach( $record as $key => $value )
                if( !is_array( $value ) )
                    ...
                    $this->parseDropdown( $dropdown_template, $value, $selected );
            $this->parseTemplate( $template, "a" );
            $this->clearTemplate( $dropdown_template );
        }      
    }    
    
    function doEmptyRecordDetection( $template, $values )
    { 
        if( !empty( $values ) )
            $this->setAttribute( $template, "visibility", "visible" );
        else 
            $this->setAttribute( "message_template", "visibility", "visible" );
    }      

    /**
     * parses a table with multiple included dropdown menues
     *
     * Works in the manner of smartTemplateParser() 
     * But if one the values is an array the method assumes an dropdown menue
     * and redirects the array to the parseDropdown() method.
     * 
     * In order to archive the parsing of arbitrary and unknown dropdown, the 
     * $values array must match some rules, to ensure that the right values
     * will be assigned to the right templates
     *
     * assumptions:
     * the purpose of one dropdown is to represent the weekdays
     * 
     * rule 1: the field that contains the array with dropdown values must have 
     * the prefix "dropdown_values_" followed by "weekdays"
     *
     * rule 2: the array which contains the dropdown values must match the
     * conditions to get parsed by the parseDropdown() method. E.g.:
     * $weekdays[0]['label'] = "Monday";
     * $weekdays[0]['value'] = "Mon";
     * $weekdays[1]['label'] = "Tuesday";
     * $weekdays[1]['value'] = "Tue";
     * etc...
     *
     * rule 3: the value of the field "weekdays" contains the string that
     * marks the selected weekday. If tuesday shall be selected the value
     * must be "Tue".
     * 
     *
     * @author  Ralf Kramer
     ...
     * @return  void
     */     
    function parseMultipleDropdownTable( $template, $values )
    { 
        $this->doEmptyRecordDetection( $template, $values );
        
        for( $i = 0; $i < count( $values ); $i++ )
        {
            // create a new by destroying the old
            $dropdown_templates = array();
            
            // alternate colors of table rows
            if( $i % 2 )
                $this->tpl->addVar( $template, "CLASS",  "tableBody" );
            else
                $this->tpl->addVar( $template, "CLASS",  "tableBodyAlt" );
        
            $record =  $values[$i];
            foreach( $record as $key => $value )
                if( !is_array( $value ) )
                    $this->tpl->addVar( $template, strtoupper( $key ), $value );
                else
                {
                    $selected_field = str_replace( "dropdown_values_", "", $key );
                    $this->parseDropdown( $selected_field, $value, $record[$selected_field] );
                    array_push( $dropdown_templates, $selected_field );
                }
            
            $this->tpl->parseTemplate( $template, "a" );
            for( $j = 0; $j < count( $dropdown_templates ); $j++ )
                $this->tpl->clearTemplate( $dropdown_templates[$j] );
