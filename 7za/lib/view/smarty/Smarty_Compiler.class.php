...
class Smarty_Compiler extends Smarty {
            ...
            if (preg_match_all('~(<\?(?:\w+|=)?|\?>|language\s*=\s*[\"\']?php[\"\']?)~is', $text_blocks[$curr_tb], $sp_match)) {
                ...
                for ($curr_sp = 0, $for_max2 = count($sp_match[1]); $curr_sp < $for_max2; $curr_sp++) {
                    if ($this->php_handling == SMARTY_PHP_PASSTHRU) {
                        ...
                        /* quote php tags */
                        $text_blocks[$curr_tb] = str_replace('%%%SMARTYSP'.$curr_sp.'%%%', htmlspecialchars($sp_match[1][$curr_sp], ENT_QUOTES, 'cp1251'), $text_blocks[$curr_tb]);
    ...
    function _parse_modifiers(&$output, $modifier_string)
    {
        ...
        for ($_i = 0, $_for_max = count($_modifiers); $_i < $_for_max; $_i++) {
            ...
            if ($_map_array) {
                if (($_modifier_name == 'htmlspecialchars') or ($_modifier_name == 'htmlentities')) {
                $output = "((is_array(\$_tmp=$output)) ? \$this->_run_mod_handler('$_modifier_name', true, \$_tmp$_modifier_args) : " . $this->_compile_plugin_call('modifier', $_modifier_name) . "(\$_tmp, ENT_QUOTES, 'cp1251'))";
                } else {
